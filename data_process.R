...
dat <- read_csv("SalesBook_2013.csv") %>%
    ...
    mutate(
        ...
        factor(levels = c("before", "after")) #default sorts by name
    )
# quality case_when
    quality = case_when(
...
# condition case_when
    condition = case_when(
        ...
        condition == "Fair" ~ -1,
        condition == "Poor" ~ -2),
    arcstyle = ifelse(is.na(arcstyle, "missing")),
    ...
    noGarage = gartype %>% str_to_lower() %>% str_detect ("") %>% as.numeric(),
) %>%
...
replace_na(c(list()))
replace_na(colMeans(select(., nocars, NUMBDRM, NUMBATHS), na.rm = TRUE)) 
